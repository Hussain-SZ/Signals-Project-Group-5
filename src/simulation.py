...
near_end_data = np.zeros_like(near_end_data) # just used for testing with far-end only
...
window_ms = 20
window_size = 1024
mse_convergence = [np.mean(error_signal[i:i+window_size] ** 2) 
                   for i in range(0, len(error_signal) - window_size, 100)]
plt.subplot(3, 1, 3)
plt.plot(mse_convergence, color='blue', linewidth=2)
