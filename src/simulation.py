...
# Note: Project recommends 16kHz, but using your file rate for consistency 
SAMPLE_RATE = fs 
...
# NLMS
weights, estimated_echo, error_signal = nlms(far_end_data, mic_input, FILTER_SIZE, STEP_SIZE, REGULARIZATION)
    
# Normalizing error signal for playback and saving
error_signal_norm = error_signal.astype(np.float32) / (np.max(np.abs(error_signal)) + 1e-10)
wavfile.write('../data/results/cleaned_output.wav', SAMPLE_RATE, np.int16(error_signal_norm * 32767))
